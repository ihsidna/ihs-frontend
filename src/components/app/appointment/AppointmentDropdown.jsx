            ...
            {loggedInUser?.id === appointmentDetails?.userId && (
              <>
                <Menu.Item>
                  {({ active }) => (
                    <Link
                      to={`/appointments/review/${appointmentDetails?.id}`}
            ...
            {(mobileAuth?.userType || userType) === userRoles.Admin && (
              <>
                <Menu.Item>
                  {({ active }) => (
                    <Link
                      to={`/allappointments/assignworker/${appointmentDetails?.id}`}
                      className={classNames(
                        active ? "bg-gray-100 text-gray-900" : "text-gray-700",
                        "block px-4 py-2 text-sm"
                      )}
            ...
            {(mobileAuth?.userType || userType) !== userRoles.User && (
              <>
                <Menu.Item>
                  {({ active }) => (
                    <Link
                      to={`/appointments/bookfollowup/${appointmentDetails?.beneficiaryId}`}
                      className={classNames(
                        active ? "bg-gray-100 text-gray-900" : "text-gray-700",
                        "block px-4 py-2 text-sm"
                      )}
                ...
                <Menu.Item>
                  {({ active }) => (
                    <Link
                      to={`/allappointments/updateappointment/${appointmentDetails?.id}`}
                      className={classNames(
                        active ? "bg-gray-100 text-gray-900" : "text-gray-700",
                        "block px-4 py-2 text-sm"
                      )}
                ...
                <Menu.Item>
                  {({ active }) => (
                    <Link
                      to={`/allappointments/updateappointment/${appointmentDetails?.id}/uploadreport`}
                      className={classNames(
                        active ? "bg-gray-100 text-gray-900" : "text-gray-700",
                        "block px-4 py-2 text-sm"
                      )}
