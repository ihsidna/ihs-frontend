...
const AssignHealthWorker = () => {
  ...
  const getHealthWorkers = useCallback(async () => {
    const response = await axiosPrivate.get("/worker/all");
  ...
  useEffect(() => {
    ...
    const getAppointment = async () => {
      try {
        ...
        isMounted && setBeneficiaryName(response.data.data[0].beneficiaryName);
  ...
  const getHealthWorkerName = () => {
    const filteredHealthWorker = healthWorkers.filter(
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      await axiosPrivate.patch(
        `/admin/appointment/${appointmentId}`,
        JSON.stringify({
          healthWorkerName: getHealthWorkerName(),
          healthWorkerId,
